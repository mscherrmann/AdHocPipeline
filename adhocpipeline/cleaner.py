...
class Cleaner():
    ...
    def clean(self,
              docs,
              return_cleaned_docs=False,
              store_cleaned_docs=True,
              remove_parsed_docs=True):
        cleaned_docs = []
        removed_sentences = []
        start = time.time()
        for announcement_idx, l in enumerate(docs["body_text"]):
            clean_doc, removed = clean_body_text(
                docs.loc[announcement_idx, "body_text"],
            ...
            if (announcement_idx + 1) % 25 == 0:
                print(
                    f"Cleaned {announcement_idx / len(docs):.2%} of "
                    f"ad-hoc announcements after "
