...
class Pipeline():
    ...
    def __init__(self,
        ...
        language : str
            Specify the language (default is "de"):            "de" : german
                                                               "en" : English
        start_date : str
            the start date for the crawler (given in the format dd.mm.yyyy).
            Only used if no index exists or if begin_from_start_date == True
        end_date : str
            the end date for the crawler (given in the format dd.mm.yyyy).
            Only used if no index exists or if begin_from_start_date == True
        begin_from_start_date : bool
            Force the crawler to begin from start date. Otherwise,
            start dat ewill be deduced from index
        topic_classification_model : str
           name of the topic classification language model, downloaded from
           the huggingface hub
        ...
        embedding_model : str
            Name of the language model used for embedding, downloaded from
            the huggingface hub
            A multilingual model is preferred, to make comparisons
            across languages
        embedding_batch_size : int
           Batch size for embedding model inference
        upload_batch_size : int
           number of samples that should be uploaded to opensearch
           index in one iteration
        tmp_storage_path : str
           path to folder from which parsed documents get loaded and to
           which cleaned documents get stored
        is_test_mode : bool
         Flag to indicate whether to use pipeline in test mode or not
         start_from : str
            Specifies from which node to start from in the pipeline.
            Requires that the respective predecessors output is stored
            in tmp_storage_path.
            possible values are: "crawler" (default),
            "parser",
            "cleaner",
            "topic_classifier",
            "embedder",
        ...
        self.__topic_classification_batch_size = \
            topic_classification_batch_size
        ...
        if self.__is_test_mode:
            ...
            self.__html_path = os.path.join(
                ...
                "AdHocHtml",
                self.__language)
        self.__client = OpenSearch(hosts=[{"host": "localhost",
                                           "port": 9200}],
        ...
        self.uploader = OpenSearchUploader(
            upload_batch_size=self.__upload_batch_size,
            tmp_storage_path=self.__tmp_storage_path,
    ...
    def update(self,
        ...
        if self.__start_from in ["crawler", "parser"]:
            ...
            if len(document_ids) == 0:
                print("No files to parse."
                      "Either the crawler did not add new files or "
                      "these files already exist in the index")
                return
            parsed_docs = self.parser.parse(
        ...
        if self.__start_from in ["crawler", "parser", "cleaner"]:
            if 'parsed_docs' not in locals():
                parsed_docs = pd.read_pickle(
                    os.path.join(
                        self.__tmp_storage_path,
                        "parsed_output.pkl"))
            cleaned_docs = self.cleaner.clean(
                parsed_docs,
        ...
        if self.__start_from in ["crawler", "parser",
                                 "cleaner", "topic_classifier"]:
            if 'cleaned_docs' not in locals():
                cleaned_docs = pd.read_pickle(
                    os.path.join(
                        self.__tmp_storage_path,
                        "cleaned_output.pkl"))
            classified_docs = \
                self.topic_classifier.topic_classification(
                    cleaned_docs,
        ...
        # Upload
        if self.__start_from in ["crawler", "parser", "cleaner",
                                 "topic_classifier", "embedder", "uploader"]:
        ...
        print(f"Time to run pipeline for {len(embedded_docs)} documents: "
    ...
    def __get_all_document_ids_index(self):
        ...
        initial_response = self.__client.search(
            index=self.__index_name,
            body=initial_query,
            scroll='2m')  # 2m is the scroll duration
        ...
        while True:
            ...
            ids.extend(
                [doc['_source']['id']
                 for doc in initial_response['hits']['hits']])

            # Get the next batch of results
            scroll_id = initial_response['_scroll_id']
            initial_response = self.__client.scroll(
                scroll_id=scroll_id,
                scroll='2m')
