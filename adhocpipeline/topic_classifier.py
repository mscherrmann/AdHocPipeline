...
class TopicClassifier():
    ...
    def __init__(self, 
        ...
        self.__label_names = ["Earnings","SEO","Management","Guidance","Gewinnwarnung",
                              "Beteiligung","Dividende","Restructuring","Debt","Law",
                              "Großauftrag","Squeeze","Insolvenzantrag","Insolvenzplan",
                              "Delay","Split","Pharma_Good","Rückkauf","Real_Invest","Delisting"]
    ...
    def topic_classification(self, 
        ...
        # Estimate topics
        start = time.time()
        topics = self.__classification_pipe(bodies.tolist(), top_k=None, batch_size = self.__batch_size)
        print(f"Topic classification of {len(docs)} ad-hoc announcements took {(time.time()-start)/60:.2f} minutes.")
        # Convert each list of dictionaries into a dictionary with labels as keys and scores as values
        topics = [{d['label']: d['score'] for d in sublist} for sublist in topics]
        # Create a DataFrame from the list of dictionaries and check which topics are active
        topics = pd.DataFrame(topics) > self.__classification_threshold
        topics = topics[self.__label_names] # Set column names to topics
        ...
        if remove_cleaned_docs:
            path_cleaned_docs = self.__tmp_storage_path + "\\cleaned_output.pkl"
            if os.path.isfile(path_cleaned_docs):
                os.remove(path_cleaned_docs)
        # Save
        if store_classified_docs:
            docs.to_pickle(self.__tmp_storage_path+"\\classified_output.pkl")
        # Return
