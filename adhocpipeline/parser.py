...
class Parser():
    ...
    def parse(
            self,
            document_ids,
            return_parsed_docs=False,
            store_parsed_docs=True):
        ...
        for announcement_idx, filename in enumerate(document_ids):
            raw_html = codecs.open(
                os.path.join(self._file_path, f"{filename}.html"), 'r').read()
        ...
        parsed_docs = parsed_docs.loc[
            parsed_docs.language == self.__language, :]
        ...
        # Some files are duplicates since they are
        # filed in different corporate forms. Remove them
