...
class Embedder():
    ...
    def __init__(self,
        ...
        ----------
        embedding_model : str
           name of the language model used for embedding,
           downloaded from the huggingface hub. A multilingual
           model is prederred, to make comparisons across languages
    ...
    def embed(self,
        ...
        start = time.time()
        sentence_embeddings = self.__embedder.encode(
            bodies.tolist(),
            batch_size=self.__batch_size).tolist()
        sentence_embeddings = pd.Series(
            sentence_embeddings,
            index=bodies.index)
        sentence_embeddings = sentence_embeddings.groupby(
            sentence_embeddings.index).agg(list)
        title_embeddings = self.__embedder.encode(
            titles.tolist(),
            batch_size=self.__batch_size).tolist()
        print(f"Embedding of {len(docs)} ad-hoc announcements "
              f"took {(time.time()-start)/60:.2f} minutes.")
