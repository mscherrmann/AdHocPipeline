...
class OpenSearchUploader():
    ...
    def __init__(self,
        ...
        upload_batch_size : int
           number of samples that should be uploaded to
           opensearch index in one iteration
    ...
    def upload(self,
               docs,
               remove_embedded_docs=True):
        # Split in document and sentence level
        sentences = docs[['hash', 'cleaned_body',
                          'topics', 'sentence_embeddings']]
        documents = docs[['title_text', 'date_text', 'time_text',
                          'category', 'hash', 'isin_all', 'isin', 'wkn',
                          'comp_name', 'file_id', 'body_text_raw',
                          'topics', 'title_embeddings']]
        # Upload sentence
        sentences = self.__prepare_sentences(sentences)
        self.__check_index("sentence")
        sentences["_id"] = sentences["hash"] + \
            sentences["sentence_id"].astype(str)
        ...
        if remove_embedded_docs:
            path_embedded_docs = os.path.join(
    ...
    def __bulk_upload(self, docs, index_name):
        ...
        while batch_idx * self.__upload_batch_size < num_docs:
            ...
            print(f"{max_idx/num_docs:.2%} of entry uploads "
                  f"done for index {index_name} after "
                  f"{(time.time()-start)/60:.2f} minutes.")
            batch_idx += 1

    def __prepare_sentences(self, sentences):
        # Explode
        sentences = sentences.explode(['cleaned_body',
                                       'topics',
                                       'sentence_embeddings'])
        # Rename columns
        sentences = sentences.rename(columns={"cleaned_body": "sentences",
                                              "topics": "topic"})
        # Add sentence id
        sentences['sentence_id'] = sentences.groupby(
            sentences.index
            ).cumcount()
        return sentences
    ...
    def __prepare_documents(self, documents):
        ...
        # Adjust date
        # Sometimes, the time of the day is only given by hours:minutes,
        # instead of hours:minutes:seconds.
        # Adjust for that first
        different_time_mask = documents.time_text.apply(len) == 5
        documents.loc[different_time_mask, "time_text"] = \
            documents.loc[different_time_mask, "time_text"] + ":00"
        documents["date"] = documents['date_text'] + \
            "T" + documents['time_text']
        documents["date"] = pd.to_datetime(
            documents["date"],
            format="%d.%m.%YT%H:%M:%S")
        documents = documents.drop(['date_text', 'time_text'], axis=1)

        # Aggregate topic
        # Function to flatten and aggregate each list of lists
        # into a single list with unique elements
        ...
        documents["topic"] = \
            documents["topic"].apply(__aggregate_list_of_lists)
