...
def parse_ad_hoc(raw_html, file_name):
    # Function to parse html files of
    # ad-hoc announcements from the Unternehmensregister.
    ...
    # The isin of the company that is given in the respective table
    doc["isin"] = ""
    doc["wkn"] = ""
    doc["comp_name"] = ""
    ...
    # Create tree of whole document
    doc_tree = BeautifulSoup(raw_html, "html.parser")
    # Extract the tree of the table with id "begin_pub"
    table_element = doc_tree.find(id='begin_pub')
    doc_text = table_element.get_text()
    table = table_element.find('table')

    # Parse all isins in the document
    isin_list = re.finditer(
        r'(AT|CA|CH|CN|DE|DK|FI|FR|GB|IL|LU|MT|NL|SE|US|XC|XF|XS)\w{9}\d',
        re.sub(r'\s{2,}',
               " ",
               doc_text)
        )
    doc["isin_all"] = list(set([x.group() for x in isin_list]))
    ...
    if not table:
        ...
        doc["body_text"] = \
            [x for x in doc["body_text"] if x != "\xa0" and x != ""]
    ...
    for i in range(len(table_element)):
        ...
        if keyword == "Betreff:":
            ...
            doc["category"] = \
                [x.strip().lower()
                 for x in re.sub(r'\s{2,}', " ", cols[1].string).split("/")]
    ...
    for i in range(len(table_element)):
        ...
        if keyword == "Name:":
            ...
            isin_pat = \
                re.finditer(
                    r'(AT|CA|CH|CN|DE|DK|FI|FR|GB|IL|LU|MT|' +
                    r'NL|SE|US|XC|XF|XS)\w{9}\d',
                    re.sub(r'\s{2,}', " ", cols[1].string))
            doc["isin"] = list(set([x.group() for x in isin_pat]))
            if len(doc["isin"]) == 1:
                doc["isin"] = doc["isin"][0]
        elif keyword == "WKN:":
    ...
    for i in range(len(table_element)):
        ...
        if (
                keyword == "Pflichtveröffentlichung in/über:" and
                doc["source_text"] == ""
        ):
            doc["source_text"] = re.sub(r'\s{2,}', " ", cols[1].string)
    ...
    doc["contains_name"] = any(
        doc["comp_name"] in paragraph for paragraph in doc["body_text"])
    if doc["title_text"] == "":
...
def extract_strings(soup):
    # This function extacts all strings (NavigableString) in a html tag,
    # no matter how many child this tag might have, separated in a list.
    strings = []
    if soup.contents:
    ...
    if soup.contents:
        for child in soup.contents:
            if isinstance(child,  NavigableString):
                ...
                strings = strings + \
                    extract_strings_and_split_at_double_line_break(child)
    return strings
